...
std::int64_t Category::getId() const
{
    return m_Id;
}

void Category::setId(std::int64_t value)
{
...
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<Category>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<Category>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<Category>>>::fromJsonValue(boost::json::parse(json));
}

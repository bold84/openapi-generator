...
| **C** | Outbound HTTP client | Non-schema OAS 3.1 client behaviour (parameter serialization, security, request/response, servers, media negotiation). | **Partial (Wave-5.1 slice).** GC1 parameter style/explode matrix golden tests PASS (19/19 cells; recording-client runtime harness `jsts_param_wire.py`): form/space/pipe/deepObject/allowReserved/allowEmptyValue query cells, simple/label/matrix path styles, header objects, AND cookie params (previously silent-dropped) — evidence `oas31-jsts/wave5-param-serialization-slice.md`. Runtime mock HTTP endpoints (5.8), GC2–GC4 and FeatureSet (5.10) remain. |

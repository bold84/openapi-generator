...
| --- | --- | --- | --- |
| **S-V** | Schema validity | Every required Core, Applicator, Validation, and Unevaluated keyword implemented with correct JSON Schema 2020-12 validity semantics, including the exact JSON Number domain. | **Not complete.** The compiled Phase-2 raw-instance runner runs locally with real accept/reject evidence and the GENERATED-path JSTS corpus is executed honestly. Waves 1-2.5 (numeric/boolean, object/array structural, pattern/string/ref) plus the **Wave-3 slice** (`wave3-multiapplicator-uneval-refremote-slice.md`) are executed: the FULL 46-file / 1299-case JSTS corpus reads **1028 PASS / 28 FAIL / 243 BLOCKED**, with **34 suites zero-FAIL AND zero-BLOCKED**: the 30 wave-2.5 suites (`additionalProperties` 21, `anyOf` 18, `boolean_schema` 18, `const` 54, `default` 7-annotation, `enum` 51, `exclusiveMaximum`/`exclusiveMinimum` 4 each, `infinite-loop-detection` 2, `items` 29, `maxItems` 6, `maxLength` 7, `maxProperties` 10, `maximum` 8, `minItems` 6, `minLength` 7, `minProperties` 10, `minimum` 11, `multipleOf` 11, `not` 40, `oneOf` 27, `pattern` 12, `patternProperties` 25, `prefixItems` 11, `properties` 28, `propertyNames` 22, `ref` 79, `required` 18, `type` 80, `uniqueItems` 69) **plus `allOf` 30, `if-then-else` 30, `dependentSchemas` 20, `refRemote` 31**. Also measured: `unevaluatedProperties` 128/1/0 (single `$dynamicRef`-interaction FAIL), `unevaluatedItems` 58/4/9 (contains/`$dynamicRef` interplay), `dynamicRef` 25/19/0 (generation-blockers cleared), all `deferred` strictly (zero-FAIL bar not met). Remaining residuals are all named, each owned by its own slice: `$dynamicRef` semantics (19), `contains` family (63 BLOCKED), format/content (annotation-only 133+18), `dependentRequired` (20), plus the single `anchor`/`defs`/`vocabulary` FAILs. GS4 zero-DEFERRED not met (19 rows external specs / round-trip M / response-dispatch / wire C-08). OAS 3.0 dual path executed-clean. |

...
| --- | --- | --- | --- |
| **S-V** | Schema validity | Every required Core, Applicator, Validation, and Unevaluated keyword implemented with correct JSON Schema 2020-12 validity semantics, including the exact JSON Number domain. | **Not complete.** The compiled Phase-2 raw-instance runner runs locally with real accept/reject evidence, and the Wave-1 COMPLETE GENERATED-path JSTS required-vocab subset (committed `oas31-jsts/wave1-complete-subset-report.md`, HEAD 36c11de5: boolean_schema 18/0/0, const 54/0/0, not 36/4/0, enum 46/5/0, uniqueItems 28/0/41 BLOCKED, ref 6/5/68 BLOCKED) plus the earlier numeric/boolean subset is real. `boolean_schema` (K-03) and `const` (K-30) are `supported` (zero-FAIL/zero-BLOCKED); composition (`allOf`/`anyOf`/`oneOf`/`discriminator`) and the numeric rows (`multipleOf`/`minimum`/`maximum`/`exclusiveMinimum`/`exclusiveMaximum`) stay `supported` on Phase-2 / JSTS evidence. `not`/`enum` are `deferred` (residual semantic FAIL though zero-BLOCKED); `uniqueItems`/`$ref` are `deferred` (BLOCKED on array-structural / top-level-`$defs` gaps that need object-array traversal). 19 of 38 semantic rows resolved; GS4 'zero DEFERRED' is not met and the full validity surface is not implemented (see §5). Per GS1/GS2, keywords whose validators are emitted but lack runtime proof are `deferred`, not `supported`. Partially attacked via pure Java dialect/structure helpers, the fail-closed surface, and the Phase-2 runner. |
...
| **`compliance-matrix.yaml`** | **Refreshed (matches reality; GS1/GS2 reclassification applied)** | `modules/openapi-generator/src/test/resources/3_1/cpp-boost-beast-client/compliance-matrix.yaml` (67 rows) defines the full row contract (`keyword`, `vocabularyUri`, `classification`, `profiles`, `status`, `acceptedSchemaDomain`, `sourceEvidence`, `parserEvidence`, `irEvidence`, `runtimeEvidence`, `jstsGroups`, `notes`) and the Wave-0 status vocabulary. Applying the GS1/GS2 runtime-only rule (`supported` requires real runtime evidence — a JSTS required-vocabulary group passing or a compiled raw-instance accept/reject proof), the matrix marks `supported` only keywords with real runtime evidence — Phase-2 composition rows (`allOf`/`anyOf`/`oneOf`/`discriminator`), the Wave-1 GENERATED-path JSTS-green numeric rows (`multipleOf`/`minimum`/`maximum`/`exclusiveMinimum`/`exclusiveMaximum`, each 0 FAIL / 0 BLOCKED in `oas31-jsts/wave1-numeric-subset-report.md`), and the Wave-1 COMPLETE GENERATED-path rows `boolean_schema` (K-03, 18/0/0) and `const` (K-30, 54/0/0) per `oas31-jsts/wave1-complete-subset-report.md`. `not`/`enum` are `deferred` (residual semantic FAIL despite zero-BLOCKED); `uniqueItems`/`$ref` are `deferred` (BLOCKED on array-structural / top-level-`$defs` gaps); `type` is `deferred` (1 type-array-null FAIL). The remaining emitted-but-unproven keywords (`properties`, `items`, `maxLength`, `minLength`, `pattern`, `maxItems`, `minItems`, `required`) are `deferred` (runtime-unproven; they do not refuse generation, so not `fail-closed`). It is explicitly a Wave-0 matrix that does **not** claim G-full-schema; EXACT-MATH / UNICODE caveats are called out per row. README table generation from the matrix is not yet wired (plan §5 item 8). |
...
#    BLOCKED: boolean_schema 18/0/0 (SUPPORTED), not 36/4/0, const 54/0/0
#    (SUPPORTED), enum 46/5/0, uniqueItems 28/0/41, ref 6/5/68; numeric rows
#    min/max/exclusiveMin/exclusiveMax/multipleOf stay green/supported; type 79/1/0.
#    zero-BLOCKED/zero-FAIL met only for boolean_schema, const, and the numeric set ->
#    those are supported; not/enum deferred (residual FAIL), uniqueItems/$ref deferred
#    (BLOCKED on object/array structural + top-level $defs gaps). GS2 not met. Required-vocab exclusions zero.
```
...
- **Reclassification (GS1/GS2 runtime-only rule):** `supported` is granted
  **only** to keywords with real runtime evidence (a JSTS required-vocabulary
  ...
  `exclusiveMinimum`, `exclusiveMaximum`), and the Wave-1 COMPLETE GENERATED-path
  rows `boolean_schema` (K-03, 18/0/0) and `const` (K-30, 54/0/0). `not`/`enum` are
  `deferred` (residual semantic FAIL despite zero-BLOCKED); `uniqueItems`/`$ref`
  are `deferred` (BLOCKED); `type` is `deferred` (1 type-array-null FAIL). The
  remaining emitted-but-unproven keywords (`properties`, `items`, `maxLength`,
  `minLength`, `pattern`, `maxItems`, `minItems`, `required`) are **deferred**
...
- **Status here:** the compiled raw-instance Phase-2 runner (K-18 / GS4) is **wired and verified locally** with Boost (`/opt/homebrew/include`), resolving 19 DEFERRED semantic rows to real accept/reject under `-Werror`; GS4 'zero DEFERRED' is **not yet fully met** (19 rows remain DEFERRED: external specs / typed round-trip M / response-dispatch / wire C-08). The JSTS suite (GS2) is **pinned at a real SHA** (`fb7372e8763a1417bddc65fa4c911b3e79b57b65`) with a Wave-0 baseline (PASS 40 / FAIL 32 / BLOCKED 316 of 388), a Wave-1 GENERATED-path numeric/boolean subset report (PASS 193 / FAIL 22 / BLOCKED 66 of 281), and a Wave-1 COMPLETE required-vocab subset report (`wave1-complete-subset-report.md`: boolean_schema 18/0/0, const 54/0/0, not 36/4/0, enum 46/5/0, uniqueItems 28/0/41, ref 6/5/68; 311 cases, 188 PASS / 14 FAIL / 109 BLOCKED) — **GS2 is not met**. The generated-path numeric/boolean wire gate is **GREEN (39/39)** and the Wave-1 regression gate `gate-wave1-complete.sh` is **GREEN (35/35)** via the real generator's `validate_<id>` dispatch. The honest authority is the **executed** JSTS subset (not the hand-authored gates): only `boolean_schema` and `const` meet zero-FAIL/zero-BLOCKED among the new rows; `not`/`enum` still carry genuine semantic inversion FAILs and `uniqueItems`/`$ref` are BLOCKED on object/array-structural + top-level-`$defs` gaps that require object-array subschema traversal. Java tests verified: `CppBoostBeastClientCodegenTest`+`ApiCodegenTest` = **105 run, 0 fail** (BUILD SUCCESS). Boost is present on this host and the parser-blockers appendix + CI job have landed (GS8/§7 in place); what remains is GS4's remainder (19 DEFERRED rows), GS2, and the object/array structural traversal needed to close the not/enum/uniqueItems/$ref residual gaps.
- **Wave-1 completion status & remaining (object/array structural traversal is the coupling):** the Wave 1 schema-validity items that close required-vocab JSTS groups and are verifiable have landed — `boolean_schema` (K-03) and `const` (K-30) `supported`; `not` (K-01), `enum` (K-34), `uniqueItems` (K-22), and `$ref`/SchemaResourceRegistry (K-29) are honestly `deferred` because their residual FAIL/BLOCKED (deep `not:{type+properties}`, object-property `enum`, object/array items + `uniqueItems:false`/`prefixItems`, and top-level `$defs` `$ref` resolution) are all **object/array structural subschema traversal** — the natural next slice. K-24 single-IR is satisfied architecturally by ADR Option B (a single IR-table `SchemaEvaluator` interpreter over densified SchemaNode/SchemaResource IR: `oas31_ir.hpp` is the sole IR source of truth; no dataType-only recovery duplication remains actionable this pass). **Deferred to further waves:** wiring of the structural-normative + dialect-gate helpers into the generation pipeline (test-suite compatibility), the full external-`$ref`/`$anchor`/cross-resource-dialect SchemaResourceRegistry (only local `$ref` landed), annotations, `unevaluated*`, `$dynamicRef`, pattern, and full raw-instance validator + outbound-C profile.

...
| --- | --- | --- | --- |
| **S-V** | Schema validity | Every required Core, Applicator, Validation, and Unevaluated keyword implemented with correct JSON Schema 2020-12 validity semantics, including the exact JSON Number domain. | **Not complete.** The compiled Phase-2 raw-instance runner runs locally and has produced real accept/reject evidence, and the Wave-1 GENERATED-path JSTS numeric/boolean subset is green for `multipleOf`/`minimum`/`maximum`/`exclusiveMinimum`/`exclusiveMaximum` (plus Phase-2-proven composition `allOf`/`anyOf`/`oneOf`/`discriminator`); 19 of 38 semantic rows resolved. But GS4 'zero DEFERRED' is not met and the full validity surface is not implemented (see §5). Per GS1/GS2, the 15 keywords whose validators are emitted but lack runtime proof are `deferred`, not `supported`. Partially attacked via pure Java dialect/structure helpers, the fail-closed surface, and the Phase-2 runner. |
...
#    BLOCKED: boolean_schema 0/0/18 (fail-closed), not 0/0/40 (fail-closed), const
#    45/9/0, enum 31/12/8, min/max/exclusiveMin/exclusiveMax/multipleOf all green
#    (11/0/0, 8/0/0, 4/0/0, 4/0/0, 11/0/0), type 79/1/0. zero-BLOCKED/zero-FAIL NOT
#    fully met -> partial support; min/max/exclusive* are supported, type/enum/const
#    deferred, boolean_schema/not fail-closed. GS2 not met. Required-vocab exclusions zero.
```
...
- **Reclassification (GS1/GS2 runtime-only rule):** `supported` is granted
  **only** to keywords with real runtime evidence (a JSTS required-vocabulary
  ...
  keywords: Phase-2 composition (`allOf`, `anyOf`, `oneOf`, `discriminator`) and
  the Wave-1 GENERATED-path JSTS-green numeric set (`multipleOf`, `minimum`,
  `maximum`, `exclusiveMinimum`, `exclusiveMaximum`). `type`/`enum`/`const` are
  `deferred` (residual JSTS FAIL/BLOCKED despite green scalar cores). The remaining
  emitted-but-unproven keywords (`$ref`, `properties`, `items`, `maxLength`,
  `minLength`, `pattern`, `maxItems`, `minItems`, `uniqueItems`, `required`)
  are **deferred** — not claimed
...
- **Status here:** the compiled raw-instance Phase-2 runner (K-18 / GS4) is **wired and verified locally** with Boost (`/opt/homebrew/include`), resolving 19 DEFERRED semantic rows to real accept/reject under `-Werror`; GS4 'zero DEFERRED' is **not yet fully met** (19 rows remain DEFERRED: external specs / typed round-trip M / response-dispatch / wire C-08). The JSTS suite (GS2) is **pinned at a real SHA** (`fb7372e8763a1417bddc65fa4c911b3e79b57b65`) with a Wave-0 baseline (PASS 40 / FAIL 32 / BLOCKED 316 of 388) and a Wave-1 GENERATED-path numeric/boolean subset report (PASS 193 / FAIL 22 / BLOCKED 66 of 281) — **GS2 is not met**. The generated-path numeric/boolean wire gate (`oas-compliance/gate-generated-path.sh`) is **GREEN (39/39)** via the real generator's `validate_<id>` dispatch; on JSTS per-file evidence `multipleOf`/`minimum`/`maximum`/`exclusiveMinimum`/`exclusiveMaximum` are `supported` (0 FAIL / 0 BLOCKED), `type`/`enum`/`const` are `deferred` (residual FAIL/BLOCKED), and `boolean_schema`/`not` stay `fail-closed` (generation rejects them). Boost is present on this host, and the parser-blockers appendix + CI job have landed (GS8/§7 items are in place); what remains unproven is GS4's remainder (19 DEFERRED rows) and GS2.
- **Deferred to further waves (Wave 1 completion + Wave 2+):** wiring of the structural-normative + dialect-gate helpers into the generation pipeline (test-suite compatibility), the full `SchemaResourceRegistry`, remaining Wave-1 keywords (annotations, `unevaluated*`, `$dynamicRef`/pattern), and full raw-instance validator + outbound-C profile.

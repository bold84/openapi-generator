...
| **C** | Outbound HTTP client | Non-schema OAS 3.1 client behaviour (parameter serialization, security, request/response, servers, media negotiation). | **Partial (Wave-5.1+5.2 slices).** GC1 parameter style/explode matrix golden tests PASS (19/19 cells) + 5.2 operation server precedence/variables goldens PASS (6/6 cells: op>path-item>root, variable defaults, trailing-slash/origin-only normalization, relative servers; parser-default `Server("/")` filtered) — evidence `oas31-jsts/wave5-param-serialization-slice.md`, `wave5-server-slice.md`, gate `jsts_param_wire.py`. Runtime mock HTTP endpoints (5.8), GC2–GC4 and FeatureSet (5.10) remain; documented seams: server-host switching + multi-server selection are caller-owned (HttpClientImpl config / constructor context). |

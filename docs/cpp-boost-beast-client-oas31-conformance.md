...
| **C** | Outbound HTTP client | Non-schema OAS 3.1 client behaviour (parameter serialization, security, request/response, servers, media negotiation). | **Partial (Wave-5.1/5.2/5.3 slices).** GC1 param matrix 19/19 + 5.2 server precedence 6/6 + **GC2 security 11/11** (metadata + pluggable `applyOperationSecurity` hook; AND/OR, `{}` anonymous, `security: []` clears, inheritance, scopes; end-to-end injected apiKey header/query assertions) — evidence `oas31-jsts/wave5-*.slice.md`, gate `jsts_param_wire.py`. Runtime mock HTTP endpoints (5.8), GC3 (callbacks/webhooks) and GC4 (content negotiation) + FeatureSet (5.10) remain; documented seams: server-host switching + multi-server selection are caller-owned (HttpClientImpl config / constructor context). |

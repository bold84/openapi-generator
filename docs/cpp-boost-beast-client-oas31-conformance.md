...
| --- | --- | --- | --- |
| **S-V** | Schema validity | Every required Core, Applicator, Validation, and Unevaluated keyword implemented with correct JSON Schema 2020-12 validity semantics, including the exact JSON Number domain. | **Not complete.** The compiled Phase-2 raw-instance runner runs locally with real accept/reject evidence and the GENERATED-path JSTS corpus is executed honestly. Wave-1 COMPLETE subset (`wave1-complete-subset-report.md`): boolean_schema 18/0/0, const 54/0/0 -> `supported`. Wave-2 STRUCTURAL subset (`wave2-structural-subset-report.md` + `wave2-verify-reproduced.md`; a committed-HEAD re-run reproduces 13 files / 378 cases = 361 PASS / 7 FAIL / 10 BLOCKED exactly): `uniqueItems` 69/0/0, `required` 18/0/0, `prefixItems` 11/0/0, `items` 29/0/0 -> `supported` (zero-FAIL AND zero-BLOCKED). Composition (`allOf`/`anyOf`/`oneOf`/`discriminator`) and numeric rows (`multipleOf`/`minimum`/`maximum`/`exclusiveMinimum`/`exclusiveMaximum`) stay `supported` on Phase-2/JSTS evidence. `not` 39/1/0, `enum` 50/1/0, `$ref` 78/1/0, `properties` 26/2/0 are zero-BLOCKED but `deferred` (residual annotation-in-not, empty-enum, remote-metaschema, patternProperties-interplay FAILs); `additionalProperties` 17/2/2 and `min/maxProperties`, `min/maxItems` (0 FAIL, 2 BLOCKED each: schema-side decimal bounds dropped at emission) are `deferred`. GS4 'zero DEFERRED' is not met and the full validity surface (annotations, `unevaluated*`, `$dynamicRef`, string length + pattern, patternProperties, propertyNames, if/then/else) is not implemented (see §5). Per GS1/GS2, keywords whose validators are emitted but lack runtime proof are `deferred`, not `supported`. The OAS 3.0 dual path is executed-clean (generation rc=0 + `-Werror` header compile, `wave2-verify-reproduced.md`). |
...
- **Status here:** the compiled raw-instance Phase-2 runner (K-18 / GS4) is **wired and verified locally** with Boost (`/opt/homebrew/include`), resolving 19 DEFERRED semantic rows to real accept/reject under `-Werror`; GS4 'zero DEFERRED' is **not yet fully met** (19 rows remain DEFERRED: external specs / typed round-trip M / response-dispatch / wire C-08). The JSTS suite (GS2) is **pinned at a real SHA** (`fb7372e8763a1417bddc65fa4c911b3e79b57b65`) with baselines Wave-0 (40/32/316 of 388), Wave-1 numeric/boolean subset (193/22/66 of 281), Wave-1 COMPLETE (311 cases: 188/14/109), and Wave-2 STRUCTURAL (13 files / **378 cases: 361 PASS / 7 FAIL / 10 BLOCKED**) — **GS2 is not met**. Executed gates are GREEN: `gate-generated-path.sh` **39/39**, `gate-wave1-complete.sh` **35/35**, `gate-a.sh --skip-build` **172 PASS / 19 DEFERRED** (`wave2-object-array-gate-a-evidence.md`); the Wave-2 corpus numbers are **reproduced exactly from committed HEAD** (`oas31-jsts/wave2-verify-reproduced.md`, rebuilt jar). OAS 3.0 dual-path executed: petstore-3.0 generation rc=0 and all 14 generated model+api headers `-Werror`-clean. Java verified: **108 run, 0 fail** (BUILD SUCCESS). The honest authority is the executed corpus: only `boolean_schema`, `const`, `uniqueItems`, `required`, `prefixItems`, `items` (plus Phase-2 composition + numeric rows) are `supported`; `not`/`enum`/`$ref`/`properties` are zero-BLOCKED but carry residual semantic FAILs; `additionalProperties`/`min/maxProperties`/`min/maxItems` are `deferred` (2 BLOCKED each on schema-side decimal bounds).

**Residual ownership (honest, mapped to named next work):**
...
| empty `enum:[]` reject-all (G14) | 1 | generator type-inference drops the empty enum | next emitter slice (numeric/structural) |
...
| schema-side decimal bounds dropped at emission (`min/maxItems`, `min/maxProperties` 1.0-form) | 8 | swagger-models Integer loses the bound lexeme | next emitter slice (decimal-bound preservation) |
| propertyNames groups | 2 | `propertyNames` unimplemented | Wave 2.5/3 (propertyNames) |
...
- **Wave-1 residual close + Wave-2 object/array structural (executed, committed):** the object/array structural subschema traversal that was the coupling for Wave-1's residual gaps has landed (engine `1a1e8c28` + emitter `ada7ba2a`): `uniqueItems`, `required`, `prefixItems`, `items` are now `supported` (zero-FAIL AND zero-BLOCKED); `not` (K-01), `enum` (K-34), `$ref`/SchemaResourceRegistry (K-29), and `properties` are now zero-BLOCKED — the Wave-1 deep-`not`, object-`enum`, `uniqueItems`-structural, and top-level-`$defs` gaps are resolved — but stay `deferred` on 1–2 residual semantic FAILs each (annotation-in-not, empty-enum, remote-metaschema, patternProperties-interplay; see the ownership table). K-24 single-IR is satisfied architecturally by ADR Option B (`oas31_ir.hpp` is the sole IR source of truth; no dataType-only recovery duplication remains actionable). **Deferred to further waves:** external/remote-`$ref` + `$anchor` + cross-resource dialect registry, schema-side decimal-bound preservation (emitter), annotations, `unevaluated*`, `$dynamicRef`, string length + pattern, patternProperties, propertyNames, dependent*/if-then-else, and the full raw-instance validator + outbound-C profile.

...
public class DynamicRefParserRetentionTest {
    ...
    public void parserRetainsDynamicRefConstructs() throws Exception {
        ...
        // reflection dump: which getters carry non-null values? (tells us where
        // $defs / $dynamicAnchor / $anchor / $id land after parsing)
        for (java.lang.reflect.Method m : s.getClass().getMethods()) {
            if (m.getParameterCount() == 0 && m.getName().matches("get.*")
                    && !"getClass".equals(m.getName())) {
                try {
                    Object v = m.invoke(s);
                    if (v != null && !(v instanceof java.util.Map)
                            && !(v instanceof java.util.List) || (m.getName().equals("getExtensions"))) {
                        System.out.println("PARSER-RETENTION " + m.getName()
                                + " -> " + String.valueOf(v).substring(0,
                                        Math.min(160, String.valueOf(v).length())));
                    }
                } catch (Exception ignored) { }
            }
        }
        System.out.println("PARSER-RETENTION full S node=" + s.toString());
    }
